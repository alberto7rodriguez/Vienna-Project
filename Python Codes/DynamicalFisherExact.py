...
def noise_kernel_residue(t, gamma, alpha1, alpha2, Temp, Nmats=20000, delta_eps=1e-10, prefactor=1/np.pi):
    ...
    nu_M = -(2.0 * np.pi * gamma**2 * alpha1 * alpha2 / beta) * mats_sum.sum(axis=0)

    out = prefactor * (nu_sys + nu_M)
    return float(out[0]) if scalar_input else out

def dnu_dT_residue(t, gamma, alpha1, alpha2, Temp, Nmats=20000, delta_eps=1e-10, prefactor=1/np.pi):
    scalar_input = np.isscalar(t)
    t = np.atleast_1d(np.asarray(t, dtype=np.float64))
    t = np.abs(t)
    ...
    out = prefactor * (nu_sys_deriv + nu_M_deriv)
...
if __name__ == "__main__":
    ...
    # sigma_M = np.array([[0.0, 0.0], [0.0, 1e6]])  # Large noise on p
    # Example 2: Heterodyne (symmetric measurement)
    sigma_M = np.array([[0.5, 0.0], 
                        [0.0, 0.5]])
    # ---------------------------------------------------------
    ...
    ax1.plot(sol["time"], sol["var_x"], label=r"$\langle x^2 \rangle$")
    ax1.plot(sol["time"], sol["var_p"], label=r"$\langle p^2 \rangle$")
    ax1.set_ylabel("Variance")
    ax1.legend()
    ax1.grid(True)
    ax1.set_title("System Dynamics")
    
    ax2.plot(sol["time"], sol["FI"], color='red', label="Fisher Information $\mathcal{F}_\sigma^G(t)$")
    ax2.set_xlabel("Time (t)")
    ax2.set_ylabel("FI")
    ax2.legend()
    ax2.grid(True)
    ax2.set_title("Dynamical Quantum Fisher Information (Temperature)")
